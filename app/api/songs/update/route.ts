...
export async function POST(req: Request) {
  try {
    ...
    if (!churchId || !songId || !song) {
      return NextResponse.json({ error: "Missing churchId, songId, or song payload" }, { status: 400 });
    }
    ...
    if (song.title !== undefined) payload.title = song.title;
    if (song.artist !== undefined) payload.artist = song.artist || null;
    if (song.key !== undefined) payload.key = song.key || null;
    if (song.tempo !== undefined) payload.tempo = song.tempo ?? null;
    if (song.time_sig !== undefined) payload.time_sig = song.time_sig || null;
    if (song.lyrics !== undefined) payload.lyrics = song.lyrics || null;
    if (song.notes !== undefined) payload.notes = song.notes || null;
    if (song.tags !== undefined) payload.tags = Array.isArray(song.tags) ? song.tags : [];
